...
def parse_vocabulary_file(input_file, output_file):
    ...
    try:
        ...
        with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerows(entries)  # 写入数据行
...
def read_dir(dirpath):
    for dirpath, dirnames, filenames in os.walk(dirpath):
        for filename in filenames:
            if not has_no_extension(filename):
            # print(filename+ '   '+str(has_no_extension(filename)))
            # if not has_no_extension(filename)
                fullname = os.path.join(dirpath, filename)
                xls=filename+'.csv'
                xls_full= os.path.join(dirpath, xls)
                parse_vocabulary_file(fullname, xls_full)

if __name__ == '__main__':


    # 示例用法
    # inf = 'word'  # 请替换为您的单词本文件名
    # ouf = 'word.csv'  # 请替换为您想要的 CSV 文件名
    # parse_vocabulary_file(inf,ouf)

    read_dir(r'D:\CODE\github\mahjong\python-mahjong\test\util\cc\fileutils\txt2csv\wd')