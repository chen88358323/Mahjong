...
import logger
import FileDetailService, dirutil, encryutil
...
class FileChecking():
    ...
    def __findFileDetail(self,path):#获取需要对比的文件完整路径
        ...
        originalcache=local_cache.load_snapshot(path)
        ...
        if originalcache is None:#无缓存直接扫描
            for dirpath, dirnames, filenames in os.walk(path):
                ...
                if dirutil.isScanDir(dirpath):
                    ...
                    fileObjList.clear()
            #存储全部缓存
            local_cache.save_snapshot(path, addcache)
        else:
            ...
            local_cache.save_snapshot(path, newCache)

    #将删除信息增加值local_cache  DB  TODO ES
    def __syncDelFile(self,originalcache,delfiles,driver):
        # 2.1 根据keylist更新删除的数据  CACHE #
        originalcache ,delcache  = local_cache.clearCacheByKeyList(originalcache, delfiles)
